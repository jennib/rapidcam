...
function arcPath(
  ...
  const sx = cx + r * Math.cos(sa),
    sy = fy(cy + r * Math.sin(sa));
  const ex = cx + r * Math.cos(ea),
    ey = fy(cy + r * Math.sin(ea));
  ...
  for (const e of (file.entities ?? []) as Record<string, unknown>[]) {
    ...
    switch (e.type) {
      ...
      case "bezier": {
        ...
        prims.push(
          `<path d="M ${n(p0.x)} ${n(fy(p0.y))} C ${n(p1.x)} ${n(fy(p1.y))} ${n(p2.x)} ${n(fy(p2.y))} ${n(p3.x)} ${n(fy(p3.y))}"/>`,
        );
        break;
      }
      case "text": {
        ...
        acc(pos.x, fy(pos.y));
        acc(pos.x + w, fy(pos.y) - size);
        const deg = n((-((e.angle as number) ?? 0) * 180) / Math.PI);
        const tf = deg !== "0" ? ` transform="rotate(${deg} ${n(pos.x)} ${n(fy(pos.y))})"` : "";
        prims.push(
          `<text x="${n(pos.x)}" y="${n(fy(pos.y))}" font-size="${n(size)}" fill="currentColor" stroke="none" font-family="sans-serif"${tf}>${esc(txt)}</text>`,
        );
        break;
      }
    }

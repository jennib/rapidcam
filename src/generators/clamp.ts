...
export const clamp: Generator = {
  ...
  build(s: Sketch): Handle[] {
    ...
    const along = s.param("along", 0, { label: "Offset along edge (0 = centred)" });
    const width = s.param("width", 60, { min: 1, label: "Width (along the edge)" });
    const reach = s.param("reach", 40, { min: 1, label: "Reach (across the edge)" });
    ...
    const overhang = s.param("overhang", 12, { min: 0, label: "Overhang onto stock" });
    const height = s.param("height", 20, { min: 0.1, label: "Height above stock top" });
    ...
    if (overhang > reach) {
      s.note(`Overhang can't exceed reach (${reach}mm) — using ${onStock}mm.`);
    }
    ...
    if (width > edgeLen) {
      s.note(`Clamp is wider than the ${Math.round(edgeLen)}mm edge it sits on.`);

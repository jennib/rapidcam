...
const onCircle = (c: Vec2, r: number, a: number): Vec2 => ({
  x: c.x + r * Math.cos(a),
  y: c.y + r * Math.sin(a),
});
...
function circlePoints(center: Vec2, r: number, ent: Entity): Vec2[] {
  ...
  if (ent instanceof ArcEntity)
    return circleCircleIntersect(center, r, ent.center, ent.radius).filter((p) =>
      angleInArc(Math.atan2(p.y - ent.center.y, p.x - ent.center.x), ent.startAngle, ent.endAngle),
    );
  const pts: Vec2[] = [];
  for (const [a, b] of entitySegs(ent))
 ...
 */
export function lineExtension(
  line: LineEntity,
  clickPos: Vec2,
  others: Entity[],
): { end: "a" | "b"; target: Vec2 } | null {
  ...
  for (const ent of others)
    for (const h of rayEntityHits(origin, dir, ent)) if (!best || h.d < best.d) best = h;
 ...
 */
export function arcExtension(
  arc: ArcEntity,
  clickPos: Vec2,
  others: Entity[],
): { end: "start" | "end"; angle: number } | null {

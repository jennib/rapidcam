...
// 3) Retract / rapid structure ------------------------------------------------
{
  ...
  check(
    "last line is safeZ retract",
    lines[lines.length - 1] === safeRetract,
...
// 4) WCS origin offset --------------------------------------------------------
{
  ...
  check(
    `rapid to p0 uses offset X: ${n(p0.x - OX)}`,
    lines.includes(`G0 X${n(p0.x - OX)} Y${n(p0.y - OY)}`),
  );
}
...
//    produced ZERO G-code with no warning). Engrave → real G3; profile → NOTE.
{
  ...
  check(
    "profiled arc emits an explanatory NOTE (not silent)",
    /NOTE: arc/.test(profileOut),
...
//     each level, descending) instead of straight plunges and all-rough-then-finish.
{
  ...
  check(
    "pocket uses a ramped entry (combined XYZ feed move)",
    hasRamp,
  ...
  // Reaches full depth.
  check(
    "pocket reaches full depth Z-3",
    all.some((l) => /Z-3(\b|\.0)/.test(l) || l.includes("Z-3")),
    "",
  );
  ...
  check(
    "finish@Z-1.5 precedes clearing@Z-3 (interleaved by depth)",
  ...
  check(
    "no straight vertical plunge to full depth",
    !straightToFull,
...
//     concentric loops should wrap the island with far fewer lifts than raster.
{
  ...
  const a = mkDoc();
  const offset = generateGCode([op("offset", a)], a.doc).split("\n");
  const b = mkDoc();
  const raster = generateGCode([op("raster", b)], b.doc).split("\n");

  const lifts = (ls: string[]) => ls.filter((l) => /^G0 Z5\b/.test(l)).length;

  check(
    "contour-parallel header present",
    offset.some((l) => l.includes("contour-parallel")),
    offset.find((l) => l.includes("clearing pass")) ?? "",
  );
  check(
    "contour-parallel uses fewer lifts than raster on an island pocket",
    lifts(offset) < lifts(raster),
    `offset lifts=${lifts(offset)}, raster lifts=${lifts(raster)}`,
  );
  check(
    "contour-parallel reaches full depth Z-2",
    offset.some((l) => l.includes("Z-2")),
    "",
  );
  check(
    "helical entry present (descending XYZ move)",
...
// 12) Finishing pass: profile gets one extra full-depth spring lap ------------
{
  ...
  check(
    "finishing pass adds exactly one extra full-depth lap",
    withFin === without + 1,
    `without=${without}, with=${withFin}`,
  );
}
...
// 13) Finishing pass: pocket gets a full-depth wall lap -----------------------
{
  ...
  const out = generateGCode([op], doc);
  check(
    "pocket finishing pass emits a full-depth wall lap",
...
// 14) Circular pocket: helical G2 entry + smooth G2 walls --------------------
{
  ...
  check(
    "circular pocket descends with a helical G2 (arc + Z)",
    all.some((l) => /^G2 .*\bZ-/.test(l)),
    all.find((l) => /^G2 /.test(l)) ?? "(no G2)",
  );
  // Walls are smooth arcs, not a faceted polyline.
  check(
    "circular pocket clears with G2 arcs",
    all.some((l) => /^G2 /.test(l)),
    "",
  );
  check(
    "circular pocket reaches full depth Z-3",
    all.some((l) => l.includes("Z-3")),
    "",
  );
  // No straight vertical plunge to full depth at the start.
  check(
    "no straight plunge to full depth",
    !all.some((l) => /^G1 Z-3\b/.test(l)),
...
// 15) Profile lead anchors mid-side only when a lead is configured -----------
{
  ...
  check(
    "mid-side start (X40) used when a lead is configured",
    /X40 Y(7|53)\b/.test(withLead),
    withLead.split("\n").find((l) => /X40/.test(l)) ?? "(no X40)",
  );
  check(
    "no mid-side start for a plain no-lead profile",
    !/X40 Y(7|53)\b/.test(noLead),
    noLead.split("\n").find((l) => /X40/.test(l)) ?? "",
  );
}
...
// 17) Chamfer: V-bit edge bevel, depth derived from width --------------------
{
  ...
  check(
    "chamfer derives depth from width (Z-5.196)",
    /Z-5\.196\b/.test(out),
    out.split("\n").find((l) => /^G1 Z-/.test(l)) ?? "",
  );
  check(
    "chamfer traces the edge",
    out.split("\n").some((l) => /^G1 X/.test(l)),
    "",
  );
  ...
  check(
    "outside chamfer offsets the edge outward (X153)",
    /X153\b/.test(outside),
  ...
  check(
    "sharpen-corners tapers to the surface at each inside corner (4)",
    liftMoves.length === 4,
    `got ${liftMoves.length}`,
  );
  ...
  check(
    "each surface lift is at a corner vertex",
    corners.every((c) => liftMoves.some((l) => l.startsWith(`G1 ${c} Z0`))),
    liftMoves.join(" | "),
  );
  check(
    "plain chamfer has no mid-contour surface lifts",
  ...
  check(
    "over-stock chamfer depth emits a NOTE",
    /NOTE: chamfer depth .* exceeds stock thickness/.test(deep),
    "",
  );
  // A sane chamfer (depth < stock) does not.
  check("normal chamfer depth emits no over-stock NOTE", !/exceeds stock thickness/.test(out), "");
  ...
  check(
    "over-wide chamfer face emits a cutting-radius NOTE",
  ...
  check(
    "chamfer within the cutting radius emits no cutting-radius NOTE",

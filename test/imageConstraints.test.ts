...
test("coincident image corner ↔ circle centre translates the image onto it", () => {
  ...
  doc.addConstraint(
    makeConstraint("fixedPoint", { points: [{ entityId: circ.id, key: "c" }], params: [120, 90] }),
  );
  doc.addConstraint(
...
test("coincident image centre ↔ circle centre centres the image on the hole", () => {
  ...
  doc.addConstraint(
    makeConstraint("fixedPoint", { points: [{ entityId: circ.id, key: "c" }], params: [100, 100] }),
  );
  doc.addConstraint(
    makeConstraint("coincident", {
      points: [
        { entityId: img.id, key: "center" },
        { entityId: circ.id, key: "c" },
      ],
    }),
  );
...
test("an unbound image is rigid under constraints — a corner constraint moves the whole image, size/angle preserved", () => {
  ...
  doc.addConstraint(
    makeConstraint("fixedPoint", { points: [{ entityId: img.id, key: "c0" }], params: [50, 50] }),
  );
  doc.addConstraint(
    makeConstraint("fixedPoint", { points: [{ entityId: img.id, key: "c2" }], params: [70, 70] }),
  );
...
test("moving the constraint target reflows the constrained image", () => {
  ...
  const fixC = makeConstraint("fixedPoint", {
...
test("DOF accounting: a rigid image has exactly 2 DOF (position); one point constraint defines it", () => {
  ...
  doc.addConstraint(
    makeConstraint("fixedPoint", { points: [{ entityId: img.id, key: "c0" }], params: [20, 20] }),
  );
...
test("a rotated image stays rigid under a corner constraint (angle + size preserved)", () => {
  ...
  doc.addConstraint(
    makeConstraint("fixedPoint", { points: [{ entityId: circ.id, key: "c" }], params: [100, 80] }),
  );
  doc.addConstraint(
    makeConstraint("coincident", {
      points: [
...
test("inconsistent two-corner pins fail gracefully — over-defined, best-fit, finite (no distort/NaN)", () => {
  ...
  doc.addConstraint(
    makeConstraint("fixedPoint", { points: [{ entityId: img.id, key: "c0" }], params: [0, 0] }),
  );
  doc.addConstraint(
    makeConstraint("fixedPoint", { points: [{ entityId: img.id, key: "c1" }], params: [50, 0] }),
  ); // implies width 50
...
test("an image-corner constraint round-trips through save/load and still solves", () => {
  ...
  doc.addConstraint(
    makeConstraint("fixedPoint", { points: [{ entityId: circ.id, key: "c" }], params: [60, 40] }),
  );
  doc.addConstraint(
    makeConstraint("coincident", {
      points: [
  ...
  expect(
    doc2.constraints.some((c) => c.type === "coincident" && c.points.some((p) => p.key === "c0")),
  ).toBe(true);

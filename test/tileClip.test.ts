...
const STRAIGHT_CUT = [
  "G21", "G90", "M3 S1000",
  "G0 Z5", "G0 X0 Y10", "G1 Z-2 F300", "G1 X100 Y10 F1000", "G0 Z5",
  "M5", "M30",
].join("\n");
...
test("a cut crossing a seam is clipped to the tile and plunges full-depth at the seam", () => {
  ...
  for (const m of moves(res.program)) {
    if (m.hasX) { expect(m.x).toBeLessThanOrEqual(50 + 1e-6); expect(m.x).toBeGreaterThanOrEqual(-1e-6); }
  }
  ...
  expect(moves(res.program).some((m) => m.motion === 1 && m.hasZ && Math.abs(m.z + 2) < 1e-6)).toBe(true);
  // Setup / spindle / end lines survive untouched.
  expect(raws(res.program)).toEqual(expect.arrayContaining(["G21", "G90", "M3 S1000", "M5", "M30"]));
});
...
test("every clipped cut stays within the tile, and arcs are linearized away", () => {
  ...
  for (const m of moves(res.program)) {
    ...
    if (m.hasX) { expect(m.x).toBeGreaterThanOrEqual(-1e-6); expect(m.x).toBeLessThanOrEqual(160 + 1e-6); }
  }
});

const DRILL = [
  "G21", "G90", "M3 S1000",
  "G0 Z5", "G0 X30 Y30", "G1 Z-5 F200", "G0 Z5",
  "M5", "M30",
].join("\n");
...
test("a drill plunge is kept when inside the tile and dropped when outside", () => {
  ...
  expect(moves(inside.program).some((m) => m.motion === 1 && m.hasZ && Math.abs(m.z + 5) < 1e-6)).toBe(true);
...
test("warns when a toolpath varies continuously in Z", () => {
  const g = [
    "G0 Z5", "G0 X0 Y0",
    "G1 Z-1 F300", "G1 X10 Y0 Z-2 F1000", "G1 X20 Y0 Z-3", "G0 Z5",
  ].join("\n");

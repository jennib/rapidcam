...
const CX = 75,
  CY = 65; // bolt-circle centre
const PCD = 80,
  HOLE_DIA = 8; // pitch-circle diameter, hole diameter
const COUNT = 6;
...
doc.addDimension(
  makeDimension("diameter", { entities: [hole.id], value: HOLE_DIA, offset: 0.6, expr: "holeDia" }),
);
...
doc.addPattern(
  makeCircularPattern(
    ...
    instanceIds,
    params,
    computeSourceSnapshot(doc.entities, [hole.id]),
  ),
);
...
console.log(
  `Wrote examples/bolt-circle.rcam — ${doc.entities.length - 1} entities, ${doc.patterns.length} pattern, ${doc.variables.length} variables`,
);

...
- **New Project: machine type** — the New Project dialog gains a **Machine type** (CNC Mill/Router | Laser) selector that sets `machineKind` up front; choosing Laser swaps the controller list to the laser posts and grays out the mill-only options (tool changer, coolant).

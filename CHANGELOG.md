...
- **CAM: Laser material test** — a "Material Test" generator (shown in the toolpath panel in laser mode) drops in a grid of engraved squares that sweep **power** (rows) against **speed** (columns), with engraved axis labels, so you can dial in settings for a new material. Each cell is an area-fill engrave op carrying its own power/feed; the geometry is grouped for easy moving/deletion.

...
### Added
- **CAM: Greyscale relief carving (CNC mill)** — the image engrave that lasers do by modulating beam power, a mill now does by modulating **Z depth**: import an image, add an **Engrave** toolpath in a mill document, and the darkness of each pixel becomes carved depth (darkest = the cut `depth`, white = the surface) — a 2.5-D relief. It needs a **ball-nose or V-bit** (the dialog defaults to a ball-nose; a flat end mill is rejected). New Cut-section controls **Relief stepover** / **dot pitch** / **Invert** plus a live **cut-time estimate** (a relief is a long job — tens of minutes to hours — so the dialog shows ~time, rows, and passes up front). The 3-D preview renders the carved relief (showing the real tool-envelope/scallop). Depth is reached over `stepdown` passes so a small bit never plunges the whole relief at once. Reuses the `image` entity and the `rasterLineInterval`/`rasterDotPitch`/`rasterInvert` op fields (documented in the [guide](docs/rcam-format-v2.md) / [schema](public/schema/rcam-v2.schema.json)). Roughing-then-finishing and tool-envelope-aware depth are deferred — for now it's a single finish pass, best for shallow reliefs.

...
- **CAM: Leaner V-carve toolpaths** — within a carved region the bit now hops between contours at a low clearance just above the stock surface instead of fully retracting to safe-Z every time. The first approach and the final park still use safe-Z (clearing fixtures); the saving is the dozens of full-height retracts between the concentric peel rings of text and shapes.

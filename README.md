...
**Engrave images can be positioned by constraints** too — an image exposes its four corners and centre as snap/pick points, so you can make a corner *coincident* with a hole, centre it on a point, or put a corner *on* a line/circle, and the image reflows to follow. By default an image is a rigid body, so a point constraint *translates* it and positioning never distorts it. Its Properties panel's **Constraints** control opens that up when you want it: **Scale to fit** (uniform resize, rotation held), **Rotate to fit**, **Scale + rotate**, or **Stretch to fit**. The main use is **calibrating a scan** — set *Scale to fit*, dimension a feature of known size on the image, and the whole image scales to match, aspect ratio exact. Prefer the narrowest mode that matches your intent: spare freedom lets the solver satisfy a constraint the wrong way (a free size can meet a "level this edge" constraint by shrinking the image away).
